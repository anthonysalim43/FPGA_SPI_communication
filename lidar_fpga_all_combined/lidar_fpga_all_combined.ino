  ...
  for (int row = 0; row < zones_per_line; row++) {
    ...
    for (int col = 0; col < zones_per_line; col++) {
        for (int target = 0; target < VL53L8CX_NB_TARGET_PER_ZONE; target++) {
            ...
            data[2]=usefull_data[row][col][target].range_sigma_mm;
            data[3]=usefull_data[row][col][target].target_status;

            for (int which_data=0;which_data<4;which_data++ ){
          // uint8_t sigma =count_count;
           //count_count++;
           //SerialPort.println(count_count);
            // for (int byte = 0; byte < 2; byte++) {
              
                crcToSend = calculateCRC8(&data[which_data], 1);
                counter_crc=0;
                pressed_once = 1;
             ...
             */
              do {
                  counter_crc++;
                  crcToSend = calculateCRC8(&data[which_data], 1);  // Calculate CRC over the data
                ...
                // SerialPort.println("Starting SPI Transmission");
                  ...
                  if (HAL_SPI_Transmit(&hspi1, &data[which_data], 1, HAL_MAX_DELAY) == HAL_OK 
